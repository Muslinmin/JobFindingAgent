...
2. **No DB access.** Adapters return `list[JobCreate]`; the pipeline POSTs to `POST /jobs`. Dedup happens in the backend.
...
`run_scrape` is a single async function that fans out queries across adapters and POSTs results to the backend. APScheduler calls it on a 24-hour tick. The scheduler holds only the cron config — all logic lives in `run_scrape` so it is testable without APScheduler.
...
The scheduler setup is:

```python
scheduler.add_job(run_scrape, "interval", hours=24, kwargs={...})
```
...
1. For each query × adapter: call `adapter.fetch(query)`, POST each `JobCreate` to `{backend_url}/jobs`
2. Wrap each `adapter.fetch()` call in try/except — one adapter failure must not abort the others
3. Wrap each POST in try/except — one POST failure must not abort remaining POSTs
4. Apply configurable `delay_s` between adapter calls
5. Log a summary per adapter per query: adapter name, query, number of jobs fetched, number of POSTs succeeded
...
test_pipeline_fans_out_across_adapters
  ...
  Mock httpx.AsyncClient capturing POST /jobs calls
  Call await run_scrape(queries=["engineer"], adapters=[mock_adapter_a, mock_adapter_b], client=mock_http, backend_url="http://test")
  Assert: mock_adapter_a.fetch called once with "engineer"
  Assert: mock_adapter_b.fetch called once with "engineer"
  Assert: POST /jobs called 4 times total (2 adapters × 2 jobs each)
...
test_pipeline_fans_out_across_queries
  ...
  Assert: POST /jobs called 4 times total
...
test_pipeline_one_adapter_failure_does_not_abort_others
  ...
  Assert: POST /jobs called 2 times (mock_adapter_b's jobs posted)
  Assert: no exception propagates out of run_scrape()
  Assert: logger.warning called at least once

test_pipeline_post_failure_does_not_abort_pipeline
  One mock adapter returning 3 JobCreate instances
  Mock HTTP client raises httpx.HTTPStatusError on the first POST, succeeds on subsequent POSTs
  Assert: no exception propagates out of run_scrape()
  Assert: remaining POSTs still attempted (called 3 times total)
  Assert: logger.warning called at least once
...
test_pipeline_empty_adapter_result
  ...
  Assert: POST /jobs never called
  Assert: no exception raised
  Assert: no warning logged

test_pipeline_posts_correct_payload
  Mock adapter returns one JobCreate with known field values
  Capture the POST body sent to /jobs
  Assert: POST body matches JobCreate.model_dump() output
```
